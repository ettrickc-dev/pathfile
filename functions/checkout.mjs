...
export default async (req) => {
  ...
  try {
    ...
    const { clientData, result, data, email, tier } = body || {};
    ...
    const attorney = tier === "attorney";
    const cents = attorney ? 19900 : 9900;
    const tierLabel = attorney ? "Attorney-Reviewed" : "Standard";
    const order = {
      id, token, email: String(email).trim(), subjectName,
      clientData, result, data,
      status: "pending", createdAt: Date.now(), tier: tierLabel, attorneyReview: attorney,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: "usd",
          product_data: { name: `Easy Immigration Filing — ${tierLabel} (${result.caseType})` },
          unit_amount: cents,
        },
        quantity: 1,
      }],
      ...
      metadata: { orderId: id },
      success_url: `${site}/app/?order=${id}&token=${token}&paid=1`,
