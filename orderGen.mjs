...
// Fetch a blank PDF from the deployed site's /blanks/ path (added by the build step).
...
// Build + store the completed PDF and email the customer. Idempotent: if the PDF
// already exists, it just (re)sends the email link. Called by the webhook AND by
// the success-page status check, so a single webhook failure never loses an order.
...
export function magicLink(order) {
  return `${SITE()}/?order=${order.id}&token=${order.token}`;
}

export async function sendDeliveryEmail(order) {
  const key = process.env.RESEND_API_KEY;
  const from = process.env.FROM_EMAIL;
  if (!key || !from) return; // email not configured yet — skip silently
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;font-size:15px;color:#16282b;line-height:1.5">
      <h2 style="color:#15514e">Your PathFile package is ready</h2>
      ...
      <hr><p style="font-size:12px;color:#5c6b66">PathFile is a self-help document-preparation service. It is not a law firm and does not provide legal advice. Confirm fees and mailing addresses at uscis.gov before filing.</p>
    </div>`;
  await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({ from, to: order.email, subject: "Your PathFile completed forms", html }),
  });
}
...
export async function sendRetrievalEmail(email, orders) {
  ...
  const from = process.env.FROM_EMAIL;
  if (!key || !from) return;
  const links = orders.map((o) => `<li><a href="${SITE()}/?order=${o.orderId}&token=${o.token}">Download package from ${new Date(o.createdAt).toLocaleDateString()}</a></li>`).join("");
  const html = `
    <div style="font-family:Arial,sans-serif;font-size:15px;color:#16282b;line-height:1.5">
      <h2 style="color:#15514e">Your PathFile downloads</h2>
  ...
  await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({ from, to: email, subject: "Your PathFile downloads", html }),
  });
}
